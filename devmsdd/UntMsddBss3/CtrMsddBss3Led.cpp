...
/******************************************************************************
 class CtrMsddBss3Led::VecVCommand
 ******************************************************************************/

utinyint CtrMsddBss3Led::VecVCommand::getTix(
			const string& sref
		) {
...
string CtrMsddBss3Led::VecVCommand::getSref(
			const utinyint tix
		) {
...
void CtrMsddBss3Led::VecVCommand::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class CtrMsddBss3Led
 ******************************************************************************/

CtrMsddBss3Led::CtrMsddBss3Led(
			UntMsdd* unt
		) : CtrMsdd(unt) {
};

utinyint CtrMsddBss3Led::getTixVCommandBySref(
			const string& sref
		) {
	return VecVCommand::getTix(sref);
};

string CtrMsddBss3Led::getSrefByTixVCommand(
			const utinyint tixVCommand
		) {
	return VecVCommand::getSref(tixVCommand);
};

void CtrMsddBss3Led::fillFeedFCommand(
			Feed& feed
		) {
	VecVCommand::fillFeed(feed);
};

Cmd* CtrMsddBss3Led::getNewCmd(
			const utinyint tixVCommand
		) {
	Cmd* cmd = NULL;
...
Cmd* CtrMsddBss3Led::getNewCmdSetTon15() {
...
Cmd* CtrMsddBss3Led::getNewCmdSetTon60() {
